        ...
        <div className="grid lg:grid-cols-[1.2fr_1fr] gap-12 lg:gap-16 items-center">
          <div className="animate-rise">
            ...
            <p className="mt-6 max-w-xl text-lg text-muted-foreground leading-relaxed">
              I'm Aarav — a freelance web developer crafting premium digital experiences for startups, creators and local brands. Custom builds and a curated marketplace of ready-made websites.
            </p>
...
const DEMOS = [
  { title: "Aurora Consulting", cat: "Business" },
  { title: "Maya Mehra", cat: "Portfolio" },
  { title: "Tabla House", cat: "Restaurant" },
  { title: "Iron Forge Gym", cat: "Gym" },
  { title: "Helix Studio", cat: "Agency" },
  { title: "Brightside Academy", cat: "School" },
  { title: "Bloom & Co.", cat: "E-commerce" },
  { title: "North Star Law", cat: "Business" },
  { title: "Ravi Kapoor", cat: "Portfolio" },
  { title: "Tandoor Nights", cat: "Restaurant" },
  { title: "Peak Performance", cat: "Gym" },
  { title: "Studio Mono", cat: "Agency" },
];
...
function DemoGallery() {
  ...
  const filtered = useMemo(
    () => DEMOS.filter((d) => (cat === "All" || d.cat === cat) && d.title.toLowerCase().includes(q.toLowerCase())),
    [q, cat]
  );
  return (
    <section id="demos" className="relative py-28 sm:py-36 bg-secondary/20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <SectionLabel>Demo Library</SectionLabel>
        <h2 className="mt-6 font-display text-4xl sm:text-5xl font-bold max-w-3xl">
          Live demos you can <span className="text-gradient-vc">click through</span> in seconds.
        </h2>

        <div className="mt-10 grid md:grid-cols-[1fr_auto] gap-4 items-start">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <input
              value={q}
              onChange={(e) => setQ(e.target.value)}
              placeholder="Search demos — bakery, agency, fitness…"
              className="w-full rounded-full glass-strong border border-border pl-11 pr-5 py-3.5 text-sm placeholder:text-muted-foreground/70 focus:outline-none focus:ring-2 focus:ring-primary/40"
            />
          </div>
            ...
            {CATEGORIES.map((c) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-xs font-semibold transition-all ${
                  cat === c ? "bg-[image:var(--gradient-brand)] text-white shadow-glow" : "glass hover:bg-secondary"
                }`}
              >
                {c}
              </button>
        ...
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((d, i) => (
            <article key={d.title} className="group rounded-3xl glass overflow-hidden hover:-translate-y-1 transition-all duration-500">
              <div className="relative aspect-[4/3] overflow-hidden" style={{ background: PROJECT_GRADIENTS[i % PROJECT_GRADIENTS.length] }}>
                <div className="absolute inset-0 bg-noise opacity-20" />
                <div className="absolute inset-6 rounded-xl bg-white/15 backdrop-blur-md border border-white/20 flex flex-col">
                  ...
                  <div className="flex-1 grid place-items-center p-3">
                    <div className="text-center">
                      <div className="font-display text-lg text-white font-bold">{d.title}</div>
                      <div className="text-[10px] text-white/70 uppercase tracking-wider mt-1">{d.cat}</div>
                    </div>
                  </div>
                </div>
                <button className="absolute inset-0 grid place-items-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="inline-flex items-center gap-2 rounded-full bg-white text-black px-5 py-2.5 text-sm font-semibold">
              ...
              <div className="p-5 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{d.cat} template</div>
                </div>
          ...
          {filtered.length === 0 && (
            <div className="col-span-full text-center py-20 text-muted-foreground">No demos match — try a different filter.</div>
          )}
        </div>
      </div>
    </section>
...
const FAQS = [
  { q: "How long does a website take?", a: "A small site lands in 5–7 days, medium in 2–3 weeks, large in 4–6 weeks. Marketplace websites go live in 48 hours after purchase." },
